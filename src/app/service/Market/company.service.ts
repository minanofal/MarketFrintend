...
export class CompanyService {
  ...
  GetCompanies(category : Category): Observable<Company[]>{
    const  url ="http://www.yarapb2a.somee.com/api/Companies/category/" + category.id;
    return this.http.get<Company[]>(url);

  }
  GetCompaniesType(type :Type): Observable<Company[]>{
    const  url ="http://www.yarapb2a.somee.com/api/Companies/type/" + type.id;
    return this.http.get<Company[]>(url);

  }

  CreateCompany(company:any): Observable<Company>{
    const  url ="http://www.yarapb2a.somee.com/api/Companies/CreateComapny/";
    return this.http.post<Company>(url,company);
  }
  DeleteCompany(company:Company) : Observable<any>{
    const url ="http://www.yarapb2a.somee.com/api/Companies/DeleteCompan/"+company.id;
    return this.http.delete<any>(url);
  }
  UpdateCompany(company : any ,  id : string) :Observable<Company>{
    const  url ="http://www.yarapb2a.somee.com/api/Companies/UpdateComapn/"+id;
    return this.http.put<Company>(url,company);
  }
  
  GetAllcategoryidCompanies(id : string): Observable<Company[]>{
    const url ="http://www.yarapb2a.somee.com/api/Companies/category/" +id;
    return this.http.get<Company[]>(url);

...
export class TypeService {
  ...
  GetTypes(category : Category): Observable<Type[]>{
    const  url ="http://www.yarapb2a.somee.com/api/types/typesofcategory/" + category.id;
    return this.http.get<Type[]>(url);

  }
  CreateType(type : TypeForm):Observable<Type>{
    const url="http://www.yarapb2a.somee.com/api/types/CreateType";
    return this.http.post<Type>(url,type,httOptiona);
  }
  DeleteType(type :Type) : Observable<string>{
    const url = "http://www.yarapb2a.somee.com/api/types/DeleteType/"+type.id;
    return this.http.delete<string>(url);
  }
  UpdateType(type :TypeForm): Observable<Type>{
    const url ="http://www.yarapb2a.somee.com/api/types/UpdateType/"+type.id;
    return this.http.put<Type>(url,type,httOptiona);
  }
  GetAllcategoryidTypes(id : string) : Observable<Type[]>{
    const  url ="http://www.yarapb2a.somee.com/api/types/typesofcategory/" +id;
    return this.http.get<Type[]>(url);
  }
}
